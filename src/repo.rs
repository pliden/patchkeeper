...
impl RepositoryUtils for Repository {
    ...
    fn path_relative_to_workdir(&self, path: &Path) -> Result<PathBuf> {
        ...
        let absolute_path = match path.to_path_buf().canonicalize() {
            Ok(path) => path,
        ...
        let relative_path = match absolute_path.strip_prefix(workdir) {
            ...
            _ => bail!("file is not in repository: {}", path.display()),
        };
    ...
    fn paths_relative_to_workdir(&self, paths: &[PathBuf]) -> Result<Vec<PathBuf>> {
        ...
        for path in paths {
            let relative_path = self.path_relative_to_workdir(Path::new(&path))?;
...
pub trait IndexUtils {
    fn unresolved_conflicts(&self) -> Result<Vec<PathBuf>>;
}
    ...
    fn unresolved_conflicts(&self) -> Result<Vec<PathBuf>> {
        ...
        if self.has_conflicts() {
            for conflict in self.conflicts()? {
                ...
                let path = PathBuf::from(String::from_utf8(index_entry.path)?);
                paths.push(path);
            }
        }
